 ...
 * pertenece actualmente al conjunto. Se modifica in-place.
 * Implementa el Algoritmo 2 e-Closure(T) de la Practica 2
 */
void epsilon_closure(
    nfa n,
    bool *states // actua como la entrada T y la salida C del algoritmo
)
{
    /*
     * Utilizamos una pila inicializada con todos los elementos de T
     * para recorrer los estados alcanzables mediante transiciones epsilon.
     */
    ...
    /*
     * Como states ya tiene los valores iniciales de T, solo metemos a la pila 
     * todos los estados que ya forman parte del conjunto.
     * C <- T
     */
    ...
    while (top >= 0)
    {
        // current <- desapilar(Pila) extraemos el estado a evaluar
        int current = stack[top--];

        // Recorremos todas las transiciones u del autómata
        // for all u en delta_N(current, epsilon)
        for (int i = 0; i < n.transition_count; i++)
        {
            transition t = n.transitions[i];

            /*
             * Buscamos únicamente transiciones epsilon que salgan del estado actual.
             */
            if (
                t.epsilon &&
                t.from == current
            ...
            {
                /*
                 * Si todavía no habíamos visitado el destino, lo agregamos.
                 * si u no esta en C
                 */
                if (!states[t.to])
                {
                    states[t.to] = true; //C <- C U {u}

                    stack[++top] = t.to; //apilar(Pila,u)
                }
            }
        }
    }
    ...
    free(stack);
    // el return esta implicito porque modificamos el puntero states
}
